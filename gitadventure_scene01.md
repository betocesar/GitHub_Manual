Manual do Usuário Git (//version https://git-scm.com/docs/user-manual/1.8.5 in 03032018)
...
A tree can be created with git-write-tree[1] and its data can be
accessed by git-ls-tree[1].  Two trees can be compared with
git-diff-tree[1].

A tag is created with git-mktag[1], and the signature can be
verified by git-verify-tag[1], though it is normally simpler to
use git-tag[1] for both.
...
[[def_pull]]pull::
	...
	<<def_merge,merge>> it.  See also git-pull[1].
